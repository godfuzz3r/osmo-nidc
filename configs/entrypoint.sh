...
if [[ $(yq -rM '.egprs."proxy-enabled"' /configs/config.yml) = "true" ]]; then
    iptables -t nat -D PREROUTING -i apn0 -p tcp -j REDIRECT --to-ports 12346 2>/dev/null
    iptables -t nat -A PREROUTING -i apn0 -p tcp -j REDIRECT --to-ports 12346

    cp /configs/redsocks/redsocks.conf /etc/redsocks.conf
    local_ip=$(yq -rM '.egprs."ip-ifconfig"' /configs/config.yml | awk -F'/' '{ print $1 }')
    sed "s@#local_ip#@$local_ip@g" -i /etc/redsocks.conf
    redsocks -c /etc/redsocks.conf

    proxy_url=$(yq -rM '.egprs."proxy-url"' /configs/config.yml)
    ./gost -L 127.0.0.1:1088 -F "$proxy_url" &
    # TODO: ipv6 routing to internet
elif [[ $(yq -rM '.egprs."routing-enabled"' /configs/config.yml) = "true" ]]; then

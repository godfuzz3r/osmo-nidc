...
class IPA(object):
    ...
    MSGT = dict(
        PING=0x00, PONG=0x01, ID_GET=0x04, ID_RESP=0x05, ID_ACK=0x06, SCCP_OLD=0xFF
    )
    _IDTAG = dict(
        SERNR=0,
        UNITNAME=1,
    ...
    def del_header(self, data):
        ...
        if (
            self.PROTO["OSMO"] == proto or self.PROTO["CCM"] == proto
        ):  # there's extension which we have to unpack
            return struct.unpack(">HBB", data[:4]) + (
                data[4:],
            )  # length, protocol, extension, data
    ...
    def skip_traps(self, data):
        ...
        if length == 0 or payload[: (length + 3)].decode("utf-8").startswith(
            self.CTRL_TRAP
        ):
    ...
    def identity(
        ...
        return (
            self.tag_unit(unit)
            + self.tag_mac(mac)
            + self.tag_loc(location)
            + self.tag_type(utype)
            + self.tag_equip(equip)
            + self.tag_sw(sw)
            + self.tag_name(name)
            + self.tag_serial(serial)
        )
...
class Ctrl(IPA):
    ...
    def add_header(self, data):
        ...
        Add CTRL header
        """
        return super(Ctrl, self).add_header(
            data.encode("utf-8"), IPA.PROTO["OSMO"], IPA.EXT["CTRL"]
        )
    ...
    def trap(self, var, val):
        ...
        return self.add_header(
            "%s %d %s %s" % (self.CTRL_TRAP, self.CTRL_TRAP_ID, var, val)
        )
    ...
    def reply(self, op_id, var, val=None):
        ...
        if val is not None:
            return self.add_header(
                "%s_%s %s %s %s" % (self.CTRL_SET, self.CTRL_REP, op_id, var, val)
            )
        return self.add_header(
            "%s_%s %s %s" % (self.CTRL_GET, self.CTRL_REP, op_id, var)
        )
